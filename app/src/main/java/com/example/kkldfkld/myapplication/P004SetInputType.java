...
public class P004SetInputType extends Activity  implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editTextP004.setInputType(InputType.TYPE_CLASS_TEXT); //SADECE TEXT
        editText2P004.setInputType(InputType.TYPE_CLASS_NUMBER);  //baslangicta text girilmesi engellemek icin

    }

    private void viewYakala() {
        toggleButtonP004=(ToggleButton)findViewById(R.id.toggleButtonP004);
        toggleButton2P004=(ToggleButton)findViewById(R.id.toggleButton2P004);
    ...
    public void onClick(View v) {
        switch(v.getId()){
            case R.id.toggleButtonP004:
                if(toggleButtonP004.isChecked()){
                    editTextP004.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);  //tipki Top|Center da olugu gibi dusunebilirsin
                }
                else{
                    editTextP004.setInputType(InputType.TYPE_CLASS_TEXT);
                }
                break;
            case R.id.toggleButton2P004:
                if(toggleButton2P004.isChecked()){
                    editText2P004.setInputType(InputType.TYPE_CLASS_NUMBER |InputType.TYPE_NUMBER_VARIATION_PASSWORD);  //sadece text ti
                }
                else{
                    editText2P004.setInputType(InputType.TYPE_CLASS_NUMBER);
                }
                break;
